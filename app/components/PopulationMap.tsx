...
export function PopulationMap() {
  ...
  const layers = getLayersForView(layersByView, view);
  ...
  const toggleLayer = (layer: MapLayer) => setLayersByView((current) => toggleLayerForView(current, view, layer));
  const selectCityFromList = (city: CityMetric) => {
  ...
  const dataSourceLabel = rawData[0]?.source ? `${rawData[0].source.provider} · ${rawData[0].source.dataset} · муниципалитеты на 01.01.2025` : "Данные на 01.01.2025 · муниципалитеты";
    ...
    {isPlacement ? <section className="decision-layout" aria-label="Подбор площадок для баз БПЛА">
      ...
      <div className="simple-map-frame">
        <svg ref={mapRef} className={isDraggingMap ? "zoomable-map dragging" : "zoomable-map"} style={{ transform: formatViewportTransform(mapViewport) }} onWheel={handleWheelZoom} onPointerDown={startMapDrag} onPointerMove={moveMapDrag} onPointerUp={endMapDrag} onPointerCancel={endMapDrag} viewBox="0 0 1000 610" role="img" aria-label={`Муниципалитеты восемнадцати регионов Центральной России и Поволжья по показателю: ${view === "density" ? "плотность" : "население"}`}>
          ...
          {layers.boundaries ? <g data-layer="boundaries">{model.regionLayers.map((region) => <path key={`${region.region}-tint`} d={region.d} fill={region.color} className={selectedRegion === "all" || region.region === selectedRegion ? "region-tint" : "region-tint region-muted"} />)}{model.regionLayers.map((region) => <g key={`${region.region}-outline`} className={`region-outline${selectedRegion === "all" ? "" : region.region === selectedRegion ? " focused" : " muted"}`} filter={`url(#region-border-${region.region})`}><path d={region.d} fill="#000" /></g>)}{visibleRegionLabels.map((region) => <RegionLabel key={region.region} name={region.name} coordinates={region.coordinates} />)}</g> : null}
          {layers.roads ? <g data-layer="roads">{roadLines.map((road) => <path key={road.name} d={road.d ?? undefined} className={`road-line ${road.type}`} />)}</g> : null}
          {layers.cities ? <g data-layer="cities">{visibleCities.map((city) => <g key={`${city.region}:${city.name}`}><line x1={city.sx} y1={city.sy} x2={city.x} y2={city.y} stroke={city.color} className="city-leader" /><circle cx={city.sx} cy={city.sy} r="3" fill={city.color} /><g transform={`translate(${city.x}, ${city.y})`} className={selectedMapCity?.name === city.name ? "city-marker selected" : "city-marker"} role="button" tabIndex={0} aria-label={`${city.index}. ${city.name}, ${city.regionName}: ${formatNumber(city.population)} жителей`} onClick={(event) => { event.stopPropagation(); toggleMapCity(city); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); toggleMapCity(city); } }}><circle r="10" fill={city.color} /><text>{city.index}</text></g></g>)}</g> : null}
        </svg>
        <MapLayerControls view={view} layers={layers} isOpen={isLayerPanelOpen} onOpenChange={setIsLayerPanelOpen} onToggle={toggleLayer} />
      ...
      <aside className="simple-side">
        ...
        <section className="city-list"><h2>10 крупнейших городов</h2><ol>{visibleMapCities.slice(0, 10).map((city) => <li key={`${city.region}:${city.name}`}><button type="button" onClick={() => selectCityFromList(city)}><b style={{ background: city.color }}>{city.index}</b><span>{city.name}</span><em>{view === "density" ? `${formatNumber(city.districtDensity)} чел./км²` : `${formatNumber(city.population)} чел.`}</em></button></li>)}</ol></section>
      </aside>
    </section>}
  </main>;
}
